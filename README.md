...
**The module also comes with the necessary resources to allow the cluster to be used as a DMS source.** To deploy an example cluster that comes with a working networking configuration and DMS source endpoint out-of-the-box, see `examples/aurora-postgresql`.

Credentials are handled using AWS Secrets Manager to avoid storing sensitive data in plaintext.
...
| [aws_secretsmanager_secret.cluster_credentials](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/secretsmanager_secret) | resource |
| [aws_secretsmanager_secret_version.cluster_credentials](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/secretsmanager_secret_version) | resource |
...
| <a name="input_create_dms_source"></a> [create\_dms\_source](#input\_create\_dms\_source) | Whether to enable the cluster as a source for DMS. | `bool` | `true` | no |
...
| <a name="input_master_username"></a> [master\_username](#input\_master\_username) | Username for the master database user. | `string` | `"foo"` | no |
| <a name="input_parameter_group_family"></a> [parameter\_group\_family](#input\_parameter\_group\_family) | The family of the cluster parameter group. | `string` | `"aurora-postgresql15"` | no |
